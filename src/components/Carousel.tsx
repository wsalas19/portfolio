...
import { motion, PanInfo, useMotionValue, useTransform, type SpringOptions } from 'motion/react';
...
export interface CarouselProps {
  ...
  loop?: boolean;
  round?: boolean;
  renderItem?: (item: CarouselItem, index: number, itemWidth: number, trackItemOffset: number, x: ReturnType<typeof useMotionValue<number>>, transition: SpringOptions) => JSX.Element;
}
        ...
        {itemsForRender.map((item, index) => (
          renderItem ? (
            <React.Fragment key={`${item?.id ?? index}-${index}`}>
              {renderItem(item, index, itemWidth, trackItemOffset, x, effectiveTransition)}
            </React.Fragment>
          ) : (
            <CarouselItem
              key={`${item?.id ?? index}-${index}`}
